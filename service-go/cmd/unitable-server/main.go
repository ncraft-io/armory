...
import (
	...
	// This Service
	"github.com/ncraft-io/armory/service-go/internal/unitable-server"
)

//go:generate go run github.com/ncraft-io/go-evaluate/cmd/evaluate

//go:evaluate date "+%Y-%m-%d %H:%M:%S %Z"
const BuildTime = "2026-05-30 13:23:25 CST"

//go:evaluate git rev-list -1 HEAD
const GitHash = "997b24b9d3407b1b82d549d60bc6bf683a3b13ba"
...
func main() {
	...
	err := config.ScanFrom(&conf, "unitableServer", "server")
	if err != nil {
	...
	if len(GitHash) == 0 {
		logs.Infow("starting UnitableServer. you can run `go generate` before `go build`, then there will be git info here.")
	} else {
		logs.Infow("starting UnitableServer", "gitHash", GitHash, "gitBranch", GitBranch, "buildTime", BuildTime)
	}
	server.Run(conf)
}
