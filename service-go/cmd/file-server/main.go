...
import (
	...
	// This Service
	"github.com/ncraft-io/armory/service-go/internal/file-server"
)

//go:generate go run github.com/ncraft-io/go-evaluate/cmd/evaluate

//go:evaluate date "+%Y-%m-%d %H:%M:%S %Z"
const BuildTime = "2026-08-21 11:29:22 CST"
...
func main() {
	...
	err := config.ScanFrom(&conf, "fileServer", "server")
	if err != nil {
	...
	if len(GitHash) == 0 {
		logs.Infow("starting FileServer. you can run `go generate` before `go build`, then there will be git info here.")
	} else {
		logs.Infow("starting FileServer", "gitHash", GitHash, "gitBranch", GitBranch, "buildTime", BuildTime)
	}
	server.Run(conf)
}
