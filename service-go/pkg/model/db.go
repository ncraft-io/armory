...
func prepareModel(database *db.DB, value interface{}, key string) (string, error) {
	if database == nil || database.DB == nil {
		return "", fmt.Errorf("model: database is not initialized")
	}
	...
	if field == nil || field.DBName == "" {
		return "", fmt.Errorf("model: primary key %s has no database column", key)
	}
	...
	if database.Config == nil || !database.Config.DisableAutoMigrate {
		if err := database.AutoMigrate(value); err != nil {
			return "", err
		}
	}
