...
func Run(cfg nserver.Config) {
	...
	go func() {
		addr := cfg.DebugAddr[0:len(cfg.DebugAddr)-1] + "5"
		logs.Infow("begin file range server", "transport", "http", "address", addr)

		fileCfg, _ := util.GetConfig()

		m := http.NewServeMux()
		const base = "/armory/file/v1/files/"
		m.Handle(base, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			http.StripPrefix(base, http.FileServer(http.Dir(fileCfg.StaticProviderConfig.Root))).ServeHTTP(w, r)
		}))
	...
	logs.Info("file.FileServer", <-errc)
